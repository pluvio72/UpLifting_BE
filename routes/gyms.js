...
router.get("/", async (req, res) => {
	try {
		const gyms = await Gym.find({ confirmed: true }).select(
			"-_id brand name address post_code"
		);
		// console.log("gyms: ", gyms);
		return res.json({ success: true, gyms });
	} catch (error) {
		console.warn(`Error in GET /gyms, ${error.message}.`);
		return res.json({ success: false, gyms: [] });
	}
});

router.post("/request-add", async (req, res) => {
	const { brand, name, address, post_code } = req.body;
